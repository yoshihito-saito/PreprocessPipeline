...
class ChanMapCanvas(QWidget):
    ...
    def render_chanmap(self, data: dict[str, Any], *, source: Path | str) -> None:
        ...
        device_ch = np.asarray(
            data["chanMap0ind"] if "chanMap0ind" in data else np.asarray(data["chanMap"]).reshape(-1) - 1
...
class BrainRegionEditorDialog(QDialog):
    ...
    def _save_as_csv(self) -> None:
        ...
        except (AnatomicalMapError, OSError) as exc:
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._probe_assignments_automatic = True
        self._xml_skipped_channels: set[int] = set()
        self._manual_reject_channels: set[int] = set()
        self._xml_controls_key: tuple[Any, ...] | None = None
        ...
        self._channel_regions_context: tuple[str, str] | None = None
    ...
    def _build_preprocess_tab(self) -> QWidget:
        ...
        self.xml_path.textChanged.connect(self._sync_xml_controls)
        ...
        self.reject_channels.textChanged.connect(self._mark_reject_channels_dirty)
    ...
    def _build_postprocess_tab(self) -> QWidget:
        ...
        self.post_dat_path = QLineEdit()
        self.post_dat_path.setPlaceholderText("Auto: binary referenced by the selected sorting")
        browse_dat = QPushButton("Browse recording")
        browse_dat.clicked.connect(self._browse_postprocess_dat)
        recording_form.addRow("recording .dat", self._field_with_button(self.post_dat_path, browse_dat))
        ...
        recording_form.addRow(
            self._hint_label(
                "Select a corresponding raw or concatenated recording when needed. Enable the filter for raw data; "
                "leave it off for an already preprocessed recording. The recording must use the same spike timeline."
            )
        )
    ...
    def _add_probe_assignment_row(self, assignment: dict[str, Any] | None = None) -> None:
        if assignment is None:
            self._mark_probe_assignments_dirty()
        ...
        for widget in [geometry, groups, x_offset]:
            self._connect_refresh(widget)
        geometry.currentTextChanged.connect(self._mark_probe_assignments_dirty)
        groups.textChanged.connect(self._mark_probe_assignments_dirty)
        x_offset.valueChanged.connect(self._mark_probe_assignments_dirty)
    ...
    def _render_probe_assignments(self, assignments: list[dict[str, Any]]) -> None:
        ...
        self._probe_rows = []
        for assignment in assignments or [{"type": "staggered", "groups": [], "x_offset": 0}]:
    ...
    def _mark_probe_assignments_dirty(self, *_args: Any) -> None:
        if self._refresh_suspended:
            return
        self._probe_assignments_automatic = False
        self._mark_chanmap_controls_dirty()
        # Editing geometry explicitly requests a newly generated map.
        self.chanmap_path.clear()

    def _mark_reject_channels_dirty(self, *_args: Any) -> None:
        if self._refresh_suspended:
            return
        self._mark_chanmap_controls_dirty()
        try:
            entered = set(parse_int_list(self.reject_channels.text()))
        except ValueError:
            return
        self._manual_reject_channels = (
            (entered - self._xml_skipped_channels) | (entered & self._manual_reject_channels)
        )

    def _sync_xml_controls(self, *_args: Any) -> None:
        if self._refresh_suspended:
            return
        try:
            ...
            return
        self._sync_anatomical_context(settings)
        xml = settings.resolved_xml_path()
        if xml is None or not xml.exists():
            return
        stat = xml.stat()
        key = (xml.resolve(), stat.st_mtime_ns, stat.st_size)
        if key == self._xml_controls_key:
            return
        try:
            assignments, _skipped = derive_probe_assignments_from_xml(xml)
            skipped = settings.xml_excluded_channels()
        except Exception as exc:
            self._append_warning_log(f"Could not load XML channel groups: {xml}\n{exc}\n")
            return
        manual_bad = self._manual_reject_channels
        self._refresh_suspended = True
        try:
            if self._probe_assignments_automatic:
                self._render_probe_assignments(assignments)
            self._xml_skipped_channels = set(skipped)
            self.reject_channels.setText(", ".join(str(ch) for ch in sorted(manual_bad | set(skipped))))
            self._xml_controls_key = key
            self._chanmap_controls_dirty = True
    ...
    def _bad_channels_from_chanmap_data(data: dict[str, Any]) -> list[int]:
        ...
        if not np.all(np.isfinite(connected)):
            raise ValueError("chanMap connected mask must be finite.")
        device_ch = np.asarray(
            data["chanMap0ind"] if "chanMap0ind" in data else np.asarray(data["chanMap"]) - 1,
            dtype=float,
        ).reshape(-1)
        if len(connected) != len(device_ch):
            raise ValueError("chanMap connected mask and channel IDs must have equal lengths.")
        if not np.all(np.isfinite(device_ch) & (device_ch >= 0) & (device_ch == np.floor(device_ch))):
            raise ValueError("chanMap channel IDs must be nonnegative integers.")
        if len(np.unique(device_ch)) != len(device_ch):
            raise ValueError("chanMap contains duplicate channel IDs.")
        ...
        return device_ch[connected <= 0].astype(int).tolist()
    ...
    def _apply_chanmap_file_to_controls(self, path: Path) -> bool:
        ...
        try:
            ...
            map_bad = self._bad_channels_from_chanmap_data(data)
        except (OSError, ValueError, KeyError) as exc:
            QMessageBox.critical(self, "chanMap", str(exc))
            return False
        self._refresh_suspended = True
        try:
    ...
    def _load_settings_chanmap_preview(self, settings: PipelineGuiSettings) -> bool:
        ...
        try:
            explicit = self._preview_chanmap_path(settings)
            if (xml_path is None or not xml_path.exists()) and explicit is None:
            ...
            source = explicit if explicit is not None else xml_path
            stat = source.stat()
            xml_stat = xml_path.stat() if xml_path is not None and xml_path.exists() else None
            key = (
                "settings",
                source.resolve(),
                ...
                assignments_json,
                str(xml_path),
                xml_stat.st_mtime_ns if xml_stat is not None else None,
            )
            if key == self._last_chanmap_preview_key:
                return True
            data = self._current_chanmap_data(settings)
            if data is None:
                return False
            self.chanmap_canvas.render_chanmap(data, source=explicit or "current GUI settings")
            self._last_chanmap_preview_key = key
            return True
        except Exception:
            return False

    def _preview_chanmap_path(self, settings: PipelineGuiSettings) -> Path | None:
        if settings.chanmap_path.strip():
            return Path(settings.chanmap_path).expanduser()
        if self.tabs.currentIndex() == 0 and self.ephys_tabs.currentIndex() == 1:
            return settings.postprocess_chanmap_path()
        return None

    def _current_chanmap_data(self, settings: PipelineGuiSettings | None = None) -> dict[str, Any] | None:
        settings = settings or self._collect_settings()
        explicit = self._preview_chanmap_path(settings)
        basepath = settings.basepath_path
        xml_path = settings.resolved_xml_path()
        if explicit is not None:
            data = loadmat(explicit)
        elif basepath is not None and xml_path is not None and xml_path.exists():
            ...
            data = loadmat(chanmap) if chanmap is not None and chanmap.exists() else None
        if data is not None:
            rejected = set(settings.preprocess.reject_channels)
            ids = np.asarray(data["chanMap0ind"] if "chanMap0ind" in data else np.asarray(data["chanMap"]) - 1).reshape(-1)
            rejected.update(settings.xml_excluded_channels(ids.tolist()))
            connected = np.asarray(data.get("connected", np.ones(len(ids))), dtype=float).reshape(-1) > 0
            if len(ids) != len(connected):
                raise AnatomicalMapError("chanMap connected mask and channel IDs have different lengths.")
            data["connected"] = (connected & ~np.isin(ids, list(rejected))).reshape(-1, 1)
        return data
    ...
    def _current_anatomical_channel_groups(self) -> list[AnatomicalChannelGroup]:
        try:
            ...
            if xml_path is None or not xml_path.exists():
                raise AnatomicalMapError("Load the session XML before loading or editing an anatomical map.")
            groups = load_session_xml_metadata(xml_path).anatomical_groups_0based
            if not groups:
                raise AnatomicalMapError(f"No anatomical channel groups found in XML: {xml_path}")
            data = self._current_chanmap_data()
            if data is None:
                return []
            return channel_groups_from_chanmap_data(data, anatomical_groups_0based=groups)
        except (OSError, ValueError, KeyError, SyntaxError) as exc:
            raise AnatomicalMapError(str(exc)) from exc

    def _sync_anatomical_context(self, settings: PipelineGuiSettings) -> None:
        xml_path = settings.resolved_xml_path()
        context = (
            str(settings.local_output_dir or settings.basepath_path or ""),
            str(xml_path.resolve()) if xml_path is not None else "",
        )
        if self._channel_regions_context is not None and context != self._channel_regions_context:
            self._channel_regions = {}
        self._channel_regions_context = context
    ...
    def _persist_anatomical_map_to_default(self) -> Path | None:
        ...
        if not groups or not self._channel_regions:
            return None
    ...
    def _load_anatomical_map(self) -> None:
        ...
        if not path:
            return
        try:
            regions = load_anatomical_map_csv(path, groups)
        except (AnatomicalMapError, OSError, UnicodeError) as exc:
            QMessageBox.critical(self, "Anatomical map", str(exc))
            return
        self._channel_regions = regions
    ...
    def _edit_anatomical_map(self) -> None:
        ...
        if not groups:
            ...
            return
        default = self._default_anatomical_map_path()
        regions = self._channel_regions
        if not regions and default.exists():
            try:
                regions = load_anatomical_map_csv(default, groups)
            except (AnatomicalMapError, OSError, UnicodeError) as exc:
                QMessageBox.critical(self, "Anatomical map", str(exc))
                return
        dialog = BrainRegionEditorDialog(groups, regions, default, self)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        try:
            saved = save_anatomical_map_csv(default, groups, dialog.channel_regions)
        except (AnatomicalMapError, OSError) as exc:
            QMessageBox.critical(self, "Anatomical map", str(exc))
            return
    ...
    def _browse_postprocess_dat(self) -> None:
        path = self._select_open_file(
            "Select recording binary",
            self.post_dat_path.text() or self.basepath.text() or str(Path.cwd()),
            "Recording files (*.dat *.bin);;All files (*)",
        )
        if path:
            self.post_dat_path.setText(path)
    ...
    def _collect_settings(self) -> PipelineGuiSettings:
        ...
        # Validate even while the user is editing an incomplete comma-separated list.
        parse_int_list(self.reject_channels.text())
        ...
        postprocess = PostprocessGuiSettings(
            ...
            dat_path=self.post_dat_path.text().strip(),
    ...
    def _apply_settings(
        ...
        *,
        ...
        try:
            ...
            self._xml_controls_key = None
            self._probe_assignments_automatic = not bool(p.probe_assignments)
            ...
            self.post_dat_path.setText(pp.dat_path)
    ...
    def _refresh_preview(self) -> None:
        try:
            ...
            lines = [
                ...
                f"chanMap: {self._preview_chanmap_path(settings) or settings.resolved_chanmap_path() or '-'}",
            ...
            self._settings_preview_text = "\n".join(lines)
            if not self._load_settings_chanmap_preview(settings):
