    ...
    *,
    ...
    shim_path.write_text(
        (
            ...
            "  final_pass_seen=0\n"
            "  all_batches_done=0\n"
            "  near_complete_seen=0\n"
            "  last_log_activity_at=$(date +%s)\n"
            ...
            "          all_batches_done=1\n"
            "        fi\n"
            "        if (( ${BASH_REMATCH[2]} - ${BASH_REMATCH[1]} <= 100 )); then\n"
            "          near_complete_seen=1\n"
            "        fi\n"
            "      fi\n"
            ...
            "        shutdown_seen_at=0\n"
            "      fi\n"
            "      if [[ \"$line\" == *\"IdleTimeout has been reached.\"* ]]; then\n"
            "        shutdown_seen_at=$(date +%s)\n"
            "      fi\n"
            "      if (( final_pass_seen == 1 || all_batches_done == 1 || near_complete_seen == 1 )) && [[ \"$line\" == *\"Parallel pool using the 'Processes' profile is shutting down.\"* ]]; then\n"
            "        shutdown_seen_at=$(date +%s)\n"
            "      fi\n"
            "    fi\n"
            "    if (( shutdown_seen_at > 0 )); then\n"
            "      now=$(date +%s)\n"
            "      required_outputs_ready=0\n"
            "      if [[ -f spike_times.npy && -f spike_clusters.npy && -f templates.npy && -f params.py ]]; then\n"
            "        required_outputs_ready=1\n"
            "      fi\n"
            "      if (( required_outputs_ready == 1 && now - shutdown_seen_at >= 120 && now - last_log_activity_at >= 30 )); then\n"
            "        if kill -0 \"$matlab_pid\" 2>/dev/null; then\n"
            "          printf '%s\\n' \"[matlab-shim] MATLAB appears stuck after parallel pool shutdown with output files present; sending SIGTERM\" >> \"$matlab_log\"\n"
            "          kill -TERM \"$matlab_pid\" 2>/dev/null || true\n"
            "          sleep 15\n"
            "        fi\n"
            "        if kill -0 \"$matlab_pid\" 2>/dev/null; then\n"
            "          printf '%s\\n' \"[matlab-shim] MATLAB still alive; sending SIGKILL\" >> \"$matlab_log\"\n"
    ...
    try:
        ...
        if matlab_log.exists():
            ...
            if "higher compute capability" in log_tail:
                ...
                "License Manager Error -15" in log_tail
                or "support/lme/15" in log_tail
                or "Feature: Statistics_Toolbox" in log_tail
            ):
                cause = "MATLAB license server communication error (-15)"
                hint = (
                    "\nDetected MATLAB license-server error -15 while checking out Statistics Toolbox. "
                    "The sorter reached MATLAB execution, but the license server was unreachable or not running. "
                    "Check network access to the license server, verify license environment variables "
                    "(for example MLM_LICENSE_FILE), and confirm that Statistics Toolbox is available for this host."
                )
        raise RuntimeError(
